...
public class TerrariaControleur implements Initializable {
	...
	public void initMap () {
			...
			personnage.getInventaire().ajouterObjet(foreuse);
			personnage.getInventaire().ajouterObjet(foreuse);
			personnage.getInventaire().ajouterObjet(foreuse);
			personnage.getInventaire().ajouterObjet(foreuse);
	    	...
	    	for(int i=0; i<10; i++) {
	    		...
	    		if(personnage.getInventaire().getListObjet().get(i) != null) {
	    			for(int z=0; z<personnage.getInventaire().getQuantiteObjets().get(i); z++) {
			    		tileItem = new Tuile(nom,((i+z)*jeu.getMoteur().getTailleTileX())+10,15,this.images.getImage(personnage.getInventaire().getListObjet().get(i).getTag()));
			    		this.paneItemsInventaire.getChildren().add(tileItem);
			    		this.controlInvent = new ControleurInventaire(tileItem, jeu, this.personnage, personnage.getInventaire().getListObjet().get(i));
	    			}
